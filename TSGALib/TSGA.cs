...
namespace TSGALib
{
    ...
    {
        public GAMember[] pop;
        public int memSize;//number of cities+1(11)
        public int popSize;//possible solutions(e.g.100)
        ...
        public void InitializePopulation()
        {
            for (int i = 0; i < popSize; i++)
            {
                ...
                for (int j = 1; j < memSize - 1; j++)
                    if (pop[i].mem[j] == 0)//if any but first and last members are 0 display error
                        Console.WriteLine(pop[i].ToString() + ";" + i.ToString());
            }
        }
        ...
        public void EvaluatePopulation()
        {
            for (int i = 0; i < popSize; i++)
            {
                pop[i].EvaluateMember();//get fitness
            }
        }
